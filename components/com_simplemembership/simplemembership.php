...
switch ($task) {
  ...
  case 'registration':
    if ($user_configuration['allow_registration'] != '0') {
        ...
        mosRedirect(JRoute::_("index.php"), JText::_("COM_SIMPLEMEMBERSHIP_REGISTRATION_FORBIDDEN"));
    }
    break;
  case 'login':
...
function show_users() {
  ...
  if($where == ''){
      $where = " WHERE prof.profile_key='profile.file' ";
  }else{
      $where .= " AND prof.profile_key='profile.file' ";
  }
...
function add_user() {
    ...
    if($err_msg != "") {
      ...
      if (array_key_exists(0, $pr_lis)) $options[] = mosHTML::makeOption(-1, $default_group_name . ' (Free user)');

...
if(!$mosConfig_live_site) $mosConfig_live_site = JURI::root();
if(!$my) $my = JFactory::getUser();
...
if (!function_exists('find_in_cck')) {
  function find_in_cck($current){
    ...
    if($current['Itemid']){
      ...
      $current['eiid'] = $params->get('instance', $current['eiid']);
    }

    if($current['task'] == 'instance' && $current['eiid'] != '0' && !$user_id){
      $query = "SELECT fk_userid FROM #__os_cck_entity_instance WHERE eiid = '".$current['eiid']."'";
      $database->setQuery($query);
      $user_id = $database->loadResult();
    }
  ...
  $current['eiid'] = JRequest::getVar('eiid',array(0));
  if(is_array($current['eiid'])){
      $current['eiid'] = $current['eiid'][0];
  }
  ...
  switch($current_component){
    ...
    case 'com_advertisementboard':
     ...
     case 'com_os_cck':
        ...
        if($find_in['cck'] == 1){ $user_id=find_in_cck($current);}
        else{$user_id=false;}
      break; 

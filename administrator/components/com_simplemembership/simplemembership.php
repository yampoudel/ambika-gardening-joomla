...
function saveGroup() {
  if(!sms_checkReferer()){
        JError::raiseWarning(0, JText::_("COM_SIMPLEMEMBERSHIP_ERROR_REFERER_SECURITY"));
        return;
    }
  ...
  $defcurrency=array('AED','AFN','ALL','ARS','AUD','AZN','BBD','BDT','BGN','BMD','BND','BOB','BRL','BSD','BWP','BYN','BZD','CAD','CHF','CLP','CNY','COP','CRC','CZK','DKK','DOP','DZD','EGP','EUR','FJD','GBP','GTQ','HKD','HNL','HRK','HUF','IDR','ILS','INR','JMD','JOD','JPY','KES','KRW','KWD','KZT','LAK','LBP','LKR','LRD','MAD','MDL','MMK','MOP','MRO','MUR','MVR','MXN','MYR','NAD','NGN','NIO','NOK','NPR','NZD','OMR','PEN','PGK','PHP','PKR','PLN','PYG','QAR','RON','RSD','RUB','SAR','SBD','SCR','SEK','SGD','SYP','THB','TND','TOP','TRY','TTD','TWD','UAH','USD','UYU','VND','VUV','WST','XCD','XOF','YER','ZAR');
  ...
  if (count($acl_group) < 1) {
      ...
      $product_name = '';
      $link = JRequest::getVar('group_link');
      $product_id = '';
  if ($post['title'] == '') {
...
function showUsers($option) {
  ...
  . "\n WHERE sm_usersd.id = sm_users.id ) as want_jg_titles FROM #__simplemembership_users AS sm_users "  
  . "\n LEFT JOIN #__user_usergroup_map AS j_u_gr_map ON j_u_gr_map.user_id = sm_users.fk_users_id " 
  . "\n LEFT JOIN #__usergroups AS usr_gr ON usr_gr.id = j_u_gr_map.group_id " 
  . "\n LEFT JOIN #__simplemembership_groups AS sm_group ON sm_users.current_gid = sm_group.id "  
  . (count($where) ? "\nWHERE " . implode(' AND ', $where) : "") 
    ...
    foreach($list_id as $item_id) {
      ...
      if ($want_gid != 0) {
        ...
        $query="UPDATE #__simplemembership_users SET expire_date='".expireDate($usr_group)."',
                ...
                last_approved=now()
                WHERE fk_users_id='".$item_id->fk_users_id."'";
        $database->setQuery($query);
        $database->query();
      }else{
          //// Default group
...
function configure($option) {
 ...
 //********   begin add send mail for admin  ******************
  ...
  $lists['before_end_notify_days'] = '<input type="text" name="before_end_notify_days" value="' . $user_configuration['before_end_notify_days'] . '" class="inputbox" size="20" maxlength="20" title="" />';
  ...
  $lists['auto_enabled'] = mosHTML::RadioList($yesno, 'auto_enabled', 'class="inputbox"', $user_configuration['auto_enabled'], 'value', 'text');
  $lists['allow_registration'] = mosHTML::RadioList($yesno, 'allow_registration', 'class="inputbox"', $user_configuration['allow_registration'], 'value', 'text');
  $lists['show_def_jgroup'] = mosHTML::RadioList($yesno, 'show_def_jgroup', 'class="inputbox"', $user_configuration['show_def_jgroup'], 'value', 'text');
  ...
  $lists['change_user_profile'] = mosHTML :: RadioList($yesno, 'change_user_profile', 'class="inputbox"', (isset($user_configuration['change_user_profile'])) ? $user_configuration['change_user_profile'] : 1, 'value', 'text');
...
function configure_save($option) {
  ...
  $user_configuration['auto_enabled'] = mosGetParam($_POST, 'auto_enabled', 0);
  $user_configuration['allow_registration'] = mosGetParam($_POST, 'allow_registration', 0);
  $user_configuration['show_def_jgroup'] = mosGetParam($_POST, 'show_def_jgroup', 0);
  ...
  $user_configuration['change_user_profile'] = addslashes($_POST['change_user_profile']);

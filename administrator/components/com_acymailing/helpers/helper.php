...
define('ACYMAILING_HELPURL', 'https://www.acyba.com/index.php?option=com_updateme&ctrl=doc&component='.ACYMAILING_NAME.'&version=5&page=');
...
function acymailing_navigationTabs(){
    ...
    if(acymailing_isAllowed($config->get('acl_subscriber_import', 'all'))) $pages['subscriber']['IMPORT'] = array('ctrl' => 'data', 'task' => 'import');
    if(acymailing_isAllowed($config->get('acl_subscriber_export', 'all'))) $pages['subscriber']['ACY_EXPORT'] = array('ctrl' => 'data', 'task' => 'export');
    if(acymailing_isAllowed($config->get('acl_lists_filter', 'all'))) $pages['subscriber']['ACY_MASS_ACTIONS'] = array('ctrl' => 'filter', 'task' => '');

    if(acymailing_isAllowed($config->get('acl_distribution_manage', 'all'))) $pages['list']['ACY_DISTRIBUTION'] = array('ctrl' => 'action', 'task' => '');

    if(acymailing_level(2) && acymailing_isAllowed($config->get('acl_autonewsletters_manage', 'all'))){
        $pages['newsletter']['AUTONEWSLETTERS'] = array('ctrl' => 'autonews', 'task' => '');
    }
    if(acymailing_level(3) && acymailing_isAllowed($config->get('acl_campaign_manage', 'all'))){
        $pages['newsletter']['ACY_CAMPAIGNS'] = array('ctrl' => 'campaign', 'task' => '');
    }
    if(acymailing_level(3) && acymailing_isAllowed($config->get('acl_simple_sending_manage', 'all'))){
        $pages['newsletter']['SIMPLE_SENDING'] = array('ctrl' => 'simplemail', 'task' => 'edit');
    }
    if(acymailing_isAllowed($config->get('acl_templates_manage', 'all'))){
        $pages['newsletter']['ACY_TEMPLATES'] = array('ctrl' => 'template', 'task' => '');
    }
    if(acymailing_isAllowed($config->get('acl_queue_manage', 'all'))){
        $pages['newsletter']['QUEUE'] = array('ctrl' => 'queue', 'task' => '');
    }

    if(acymailing_level(1) && $config->get('anonymous_tracking', 0) == 0) $pages['stats']['DETAILED_STATISTICS'] = array('ctrl' => 'stats', 'task' => 'detaillisting');
    if(acymailing_level(1)) $pages['stats']['CLICK_STATISTICS'] = array('ctrl' => 'statsurl', 'task' => '');
    if(acymailing_level(1)) $pages['stats']['CHARTS'] = array('ctrl' => 'diagram', 'task' => '');
...
function acymailing_footer(){
	...
	$text = '<!-- '.ACYMAILING_NAME.' Component powered by https://www.acyba.com -->

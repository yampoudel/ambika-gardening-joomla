...
$document->addStyleSheet('plugins/simplemembership/plg_simplemembership_get_cck_user_instances4/buttons.css');
        ...
        if (!function_exists('showInstanceManager')){
            ...
            if (file_exists($path)){ 
                ...
                $_GLOBALS['number_of_plugin'] = 4;
                if(isset($_REQUEST['number_of_plugin']) && $_GLOBALS['number_of_plugin'] != $_REQUEST['number_of_plugin']){
                    $task = 'ShowUserInstances';
                }
                $plugin = JPluginHelper::getPlugin('simplemembership', 'plg_simplemembership_get_cck_user_instances4');
